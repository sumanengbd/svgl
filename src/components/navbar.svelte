<script lang="ts">
  ...
  import HeaderLogoLink from '@/components/headerLogoLink.svelte';

  interface NavbarProps {
    currentPath: string;
  }

  let { currentPath }: NavbarProps = $props();
...
>
  <div class="mx-auto flex items-center justify-between">
    ...
    <div class="flex items-center space-x-0 md:space-x-7">
      <div
        class="flex items-center divide-x divide-neutral-300 dark:divide-neutral-700 md:space-x-4"
      >
        {#each externalLinks as link}
          ...
          >
            {#if !link.external}
              <link.icon size={16} strokeWidth={1.5} class="mr-2" name={link.name} />
            {/if}
